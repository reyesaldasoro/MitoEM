function [xx_3D,yy_3D,zz_3D] = displayIndividualMitochondria(Hela_nuclei,invaginations,Hela_mitochondria,Hela_mitochondria_P,fstep,xx_3D,yy_3D,zz_3D)
...
maxSlice            = levs;
minSlice            = 1;
%indivValuesMito     = unique(Hela_mitochondria(:));
%%
if ~exist("fstep",'var')
    fstep=4;
end
figure
    % ***** display all the cells as surfaces in one 3D plot ****       
        ...
        for k=1:size(Hela_mitochondria_P,1)
            disp(k)
            surf_Mitochondria         = isosurface(yy_3D(1:fstep:end,1:fstep:end,minSlice:maxSlice)  ,...
                xx_3D(1:fstep:end,1:fstep:end,minSlice:maxSlice)  ,...
                zz_3D(1:fstep:end,1:fstep:end,minSlice:maxSlice)  ,...
                Hela_mitochondria(1:fstep:end,1:fstep:end,minSlice:maxSlice)==Hela_mitochondria_P.label(k),0.7);
